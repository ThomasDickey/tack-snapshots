...
MODULE_ID("$Id: color.c,v 1.6 1998/01/10 00:33:13 Daniel.Weaver Exp $")
...
{				/* print the programable color display */
	...
	if (set_foreground ? pairs_used >= max_colors :
	...
	int *ch)
{
	...
	if (set_foreground == NULL && set_color_pair == NULL) {
		ptextln("Both set foreground (setf) and set color pair (scp) are not present.");
		if (!set_background) {
			ptextln("(setb) set background not present");
		}
	...
	int *ch)
{
	...
	if (magic_cookie_glitch > 0) {
		cookie_monster =
			(set_foreground ? magic_cookie_glitch : 0) +
			(set_background ? magic_cookie_glitch : 0) +
